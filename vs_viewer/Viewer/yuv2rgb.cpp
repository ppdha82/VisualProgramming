...
*/
  ...
  if( yuv2rgb == NULL ) {
    TRACE( "No accelerated colorspace coversion found\n" );
			  ...
			  const uint_32 y_stride, const uint_32 uv_stride)
{
  ...
  uint_32 y_stride_tmp = y_stride;
#if 0
  py_line_1 = py;
  py_line_2 = py + y_stride;
#else
  y_stride_tmp = y_stride*2 - h_size;
  py_line_1 = py;
  py_line_2 = py + y_stride_tmp - (y_stride-h_size);
#endif
  ...
  for (y = 0; y < v_size / 2; y++) 
      ...
      for (x = 0; x < h_size / 2; x++) 
	{
	  ...
	  *dst_line_2++ = clip[(Y+b_common)>>16];
      ...
      py_line_1 += y_stride_tmp;
      py_line_2 += y_stride_tmp;
      pu += uv_stride - h_size/2;
      pv += uv_stride - h_size/2;
...
static void YUV2RGB420_8(uint_8* image, const uint_8* py, const uint_8* pu,
		  ...
		  const uint_32 y_stride, const uint_32 uv_stride)
{
  TRACE( "No support for 8bit displays.\n" );
}
...
static void createCLUT(uint_32 bpp, uint_32 mode) 
{
  ...
  if (lookUpTable == NULL) {
    lookUpTable = (uint_16*)malloc((1<<18)*sizeof(uint_16));
...
{  
  ...
  TRACE( "%ibpp not supported by yuv2rgb\n", bpp );
